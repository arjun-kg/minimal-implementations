...
def evaluate_episode(env, policy):
    ...
    while not done:
        env.render()
        action, hidden = policy.get_action(state, hidden, eval=True)
...
if __name__ == '__main__':
    ...
    policy = DRQN(env.observation_space.shape[0], env.action_space.n).to(device)
    ...
    for ep in range(n_episodes):
        ...
        while not done:
            ...
            if done:
                ...
                state_seq.extend([np.zeros_like(state) for _ in range(pad_len)])
